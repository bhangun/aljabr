...
class SessionListNotifier extends StateNotifier<List<Session>> {
  ...
  Future<Session> forkSession(String id, [String? messageId]) async {
    ...
    // Copy parent chat history to forked session
    final parentEntries = _ref.read(chatTranscriptProvider(id));
    if (parentEntries.isNotEmpty) {
      ...
      _ref
          .read(chatTranscriptProvider(forkedId).notifier)
    ...
    try {
      ...
      if (backendForked.id != forkedId) {
        final copied = _ref.read(chatTranscriptProvider(forkedId));
        if (copied.isNotEmpty) {
          _ref
              .read(chatTranscriptProvider(backendForked.id).notifier)
              .setEntries(copied);
        }
      }
