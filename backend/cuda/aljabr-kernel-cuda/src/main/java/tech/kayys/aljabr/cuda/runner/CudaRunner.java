package tech.kayys.aljabr.cuda.runner;
...
import tech.kayys.aljabr.cuda.config.CudaRunnerMode;
import tech.kayys.aljabr.cuda.detection.CudaDetector;
import tech.kayys.aljabr.cuda.detection.CudaCapabilities;
import tech.kayys.aljabr.cuda.optimization.CudaOptimizationManager;
 ...
 * {@link tech.kayys.aljabr.extension.metal.runner.MetalRunner}:
 * <ol>
 *   <li>RMS Norm — {@code aljabr_cuda_rmsnorm}</li>
 *   <li>QKV projection — {@code aljabr_cuda_matmul} (cuBLAS/cuBLASLt)</li>
 *   <li>FlashAttention-2/3 — {@code aljabr_cuda_flash_attn_v2/v3}</li>
 *   <li>SiLU FFN — {@code aljabr_cuda_silu_ffn}</li>
 * </ol>
 *
 ...
 * <h3>Config</h3>
 * <pre>
 *   aljabr.runners.cuda.enabled=false
 *   aljabr.runners.cuda.mode=auto  # auto|standard|offload|force|disabled
 *   aljabr.runners.cuda.library-path=/usr/local/cuda/lib64/libaljabr_cuda.so
 *   aljabr.runners.cuda.device-id=0
 *   aljabr.runners.cuda.num-layers=32
 *   aljabr.runners.cuda.num-heads=32
 *   aljabr.runners.cuda.num-heads-kv=8
 *   aljabr.runners.cuda.head-dim=128
 *   aljabr.runners.cuda.model-dim=4096
 *   aljabr.runners.cuda.ffn-dim=14336
 *   aljabr.runners.cuda.vocab-size=32000
 * </pre>
 *
 * <h3>Build CUDA kernels</h3>
 * <pre>
 *   make -C src/main/cpp/cuda CUDA_ARCH=sm_80
 *   # Output: target/native/linux-x86_64/aljabr_cuda_sm80.cubin
 * </pre>
 */
@ApplicationScoped
public class CudaRunner extends AbstractAljabrRunner {
    ...
    @ConfigProperty(name = "aljabr.runners.cuda.enabled",      defaultValue = "false")
    boolean enabled;

    @ConfigProperty(name = "aljabr.runners.cuda.mode",         defaultValue = "auto")
    String cudaMode;

    @ConfigProperty(name = "aljabr.runners.cuda.library-path",
                    defaultValue = "/usr/local/cuda/lib64/libaljabr_cuda.so")
    String libraryPath;

    @ConfigProperty(name = "aljabr.runners.cuda.device-id",    defaultValue = "0")
    int deviceId;

    @ConfigProperty(name = "aljabr.runners.cuda.num-layers",   defaultValue = "32")
    int numLayers;

    @ConfigProperty(name = "aljabr.runners.cuda.num-heads",    defaultValue = "32")
    int numHeads;

    @ConfigProperty(name = "aljabr.runners.cuda.num-heads-kv", defaultValue = "8")
    int numHeadsKv;

    @ConfigProperty(name = "aljabr.runners.cuda.head-dim",     defaultValue = "128")
    int headDim;

    @ConfigProperty(name = "aljabr.runners.cuda.model-dim",    defaultValue = "4096")
    int modelDim;

    @ConfigProperty(name = "aljabr.runners.cuda.ffn-dim",      defaultValue = "14336")
    int ffnDim;

    @ConfigProperty(name = "aljabr.runners.cuda.vocab-size",   defaultValue = "32000")
    int vocabSize;
    ...
    public void initialize(ModelManifest modelManifest, RunnerConfiguration config)
        ...
        if (!enabled)
            throw new RunnerInitializationException(
                    ...
                    "CUDA runner disabled (aljabr.runners.cuda.enabled=false)");
        if (mode == CudaRunnerMode.DISABLED)
            throw new RunnerInitializationException(
                    ErrorCode.INIT_NATIVE_LIBRARY_FAILED.name(),
                    "CUDA runner disabled (aljabr.runners.cuda.mode=disabled)");
        if (mode == CudaRunnerMode.OFFLOAD)
            throw new RunnerInitializationException(
                    ErrorCode.INIT_NATIVE_LIBRARY_FAILED.name(),
                    "CUDA runner disabled (aljabr.runners.cuda.mode=offload)");
