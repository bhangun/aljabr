...
```properties
# Enable Metal runner
aljabr.runners.metal.enabled=true

# Runner mode: auto|standard|offload|force|disabled
aljabr.runners.metal.mode=auto
...
# Output
target/native/macos-aarch64/libaljabr_metal.dylib

# Install to default location
mkdir -p ~/.aljabr/libs
cp target/native/macos-aarch64/libaljabr_metal.dylib ~/.aljabr/libs/
```
...
│                   MetalBinding (FFM)                    │
├─────────────────────────────────────────────────────────┤
│  libaljabr_metal.dylib → Metal → Apple GPU             │
...
```java
import tech.kayys.aljabr.metal.runner.MetalRunner;
import tech.kayys.aljabr.metal.optimization.MetalOptimizationManager;
...
System.setProperty("aljabr.runners.metal.mode", "offload");
...
aljabr.runners.metal.mode=offload

# Or reduce model size
aljabr.runners.metal.num-layers=16  # Instead of 32
```
