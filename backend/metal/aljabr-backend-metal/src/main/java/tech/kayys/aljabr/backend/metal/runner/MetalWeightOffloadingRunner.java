 ...
 * <h3>Config</h3>
 * <pre>
 *   aljabr.runners.metal-offload.enabled=false
 *   aljabr.runners.metal.mode=auto  # auto|standard|offload|force|disabled
 *   aljabr.runners.metal-offload.library-path=~/.aljabr/libs/libaljabr_metal.dylib
 *   aljabr.runners.metal-offload.hot-layers=8
 *   aljabr.runners.metal-offload.pin-refresh-interval-ms=10000
 *   aljabr.runners.metal-offload.num-layers=32
 *   aljabr.runners.metal-offload.model-dim=4096
 *   aljabr.runners.metal-offload.head-dim=128
 *   aljabr.runners.metal-offload.num-heads=32
 *   aljabr.runners.metal-offload.num-heads-kv=8
 *   aljabr.runners.metal-offload.ffn-dim=14336
 * </pre>
 */
@ApplicationScoped
public class MetalWeightOffloadingRunner extends AbstractAljabrRunner {
    ...
    @ConfigProperty(name = "aljabr.runners.metal-offload.enabled",              defaultValue = "false")
    boolean enabled;

    @ConfigProperty(name = "aljabr.runners.metal.mode",                        defaultValue = "auto")
    String metalMode;

    @ConfigProperty(name = "aljabr.runners.metal-offload.library-path",
                    defaultValue = "~/.aljabr/libs/libaljabr_metal.dylib")
    String libraryPath;

    @ConfigProperty(name = "aljabr.runners.metal-offload.hot-layers",           defaultValue = "8")
    int hotLayers;

    @ConfigProperty(name = "aljabr.runners.metal-offload.pin-refresh-interval-ms", defaultValue = "10000")
    long pinRefreshIntervalMs;

    @ConfigProperty(name = "aljabr.runners.metal-offload.num-layers",           defaultValue = "32")
    int numLayers;

    @ConfigProperty(name = "aljabr.runners.metal-offload.model-dim",            defaultValue = "4096")
    int modelDim;

    @ConfigProperty(name = "aljabr.runners.metal-offload.head-dim",             defaultValue = "128")
    int headDim;

    @ConfigProperty(name = "aljabr.runners.metal-offload.num-heads",            defaultValue = "32")
    int numHeads;

    @ConfigProperty(name = "aljabr.runners.metal-offload.num-heads-kv",         defaultValue = "8")
    int numHeadsKv;

    @ConfigProperty(name = "aljabr.runners.metal-offload.ffn-dim",              defaultValue = "14336")
    int ffnDim;
    ...
    public void initialize(ModelManifest modelManifest, RunnerConfiguration config)
        ...
        if (mode == MetalRunnerMode.DISABLED) throw new RunnerInitializationException(
                ErrorCode.INIT_NATIVE_LIBRARY_FAILED.name(),
                "MetalWeightOffload runner disabled (aljabr.runners.metal.mode=disabled)");
        if (mode == MetalRunnerMode.STANDARD) throw new RunnerInitializationException(
                ErrorCode.INIT_NATIVE_LIBRARY_FAILED.name(),
                "MetalWeightOffload runner disabled (aljabr.runners.metal.mode=standard)");
