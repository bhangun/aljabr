package tech.kayys.aljabr.metal.runner;
...
import tech.kayys.aljabr.metal.config.MetalRunnerMode;
import tech.kayys.aljabr.metal.detection.AppleSiliconDetector;
import tech.kayys.aljabr.metal.detection.MetalCapabilities;
 ...
 *       {@code aljabr_metal_fa4_attention} (MPSGraph SDPA on macOS 14+).</li>
 ...
 * {@code aljabr_metal_buffers.m} wraps pointers with {@code newBufferWithBytesNoCopy}
 * to give Metal a {@code MTLStorageModeShared} view — <b>zero copy</b>.
 * Aljabr's K/V pool ({@link tech.kayys.aljabr.kvcache.PhysicalBlockPool#rawKPool()})
 * is already in off-heap memory, so KV cache accesses are zero-copy too.
 *
 * <h3>Config</h3>
 * <pre>
 *   aljabr.runners.metal.enabled=false
 *   aljabr.runners.metal.mode=auto  # auto|standard|offload|force|disabled
 *   aljabr.runners.metal.library-path=~/.aljabr/libs/libaljabr_metal.dylib
 *   aljabr.runners.metal.num-layers=32
 *   aljabr.runners.metal.num-heads=32
 *   aljabr.runners.metal.num-heads-kv=8
 *   aljabr.runners.metal.head-dim=128
 *   aljabr.runners.metal.model-dim=4096
 *   aljabr.runners.metal.ffn-dim=14336
 *   aljabr.runners.metal.vocab-size=32000
 * </pre>
 *
...
public class MetalRunner extends AbstractAljabrRunner {
    ...
    @ConfigProperty(name = "aljabr.runners.metal.enabled",      defaultValue = "false")
    boolean enabled;

    @ConfigProperty(name = "aljabr.runners.metal.mode",         defaultValue = "auto")
    String metalMode;

    @ConfigProperty(name = "aljabr.runners.metal.library-path",
                    defaultValue = "~/.aljabr/libs/libaljabr_metal.dylib")
    String libraryPath;

    @ConfigProperty(name = "aljabr.runners.metal.num-layers",   defaultValue = "32")
    int numLayers;

    @ConfigProperty(name = "aljabr.runners.metal.num-heads",    defaultValue = "32")
    int numHeads;

    @ConfigProperty(name = "aljabr.runners.metal.num-heads-kv", defaultValue = "8")
    int numHeadsKv;

    @ConfigProperty(name = "aljabr.runners.metal.head-dim",     defaultValue = "128")
    int headDim;

    @ConfigProperty(name = "aljabr.runners.metal.model-dim",    defaultValue = "4096")
    int modelDim;

    @ConfigProperty(name = "aljabr.runners.metal.ffn-dim",      defaultValue = "14336")
    int ffnDim;

    @ConfigProperty(name = "aljabr.runners.metal.vocab-size",   defaultValue = "32000")
    int vocabSize;
    ...
    public void initialize(ModelManifest modelManifest, RunnerConfiguration config)
        ...
        if (!enabled) throw new RunnerInitializationException(
                ...
                "Metal runner disabled (aljabr.runners.metal.enabled=false)");
        if (mode == MetalRunnerMode.DISABLED) throw new RunnerInitializationException(
                ErrorCode.INIT_NATIVE_LIBRARY_FAILED.name(),
                "Metal runner disabled (aljabr.runners.metal.mode=disabled)");
        if (mode == MetalRunnerMode.OFFLOAD) throw new RunnerInitializationException(
                ErrorCode.INIT_NATIVE_LIBRARY_FAILED.name(),
                "Metal runner disabled (aljabr.runners.metal.mode=offload)");
        ...
        if (metal.isNativeAvailable()) {
            int err = metal.init(); // aljabr_metal_init() — MTLCreateSystemDefaultDevice
            if (err != 0) throw new RunnerInitializationException(
                    ErrorCode.INIT_NATIVE_LIBRARY_FAILED.name(),
                    "aljabr_metal_init() returned " + err);
            if (!metal.isRuntimeActive()) throw new RunnerInitializationException(
                    ErrorCode.INIT_NATIVE_LIBRARY_FAILED.name(),
                    "aljabr_metal_init() fell back to CPU; native Metal runtime is not active");
        }
