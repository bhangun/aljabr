/**
 * aljabr_metal_mps_validation.m — correctness checks for optional MPS matvec paths.
 */
...
static float half_matvec_reference_row(const float* A, const uint16_t* B, int K, int row) {
    ...
    float sum = 0.0f;
    for (int k = 0; k < K; k++) {
        sum += A[k] * aljabr_metal_f16_to_f32(weight[k]);
    }
    return sum;
}
...
static float bf16_matvec_reference_row(const float* A, const uint16_t* B, int K, int row) {
    ...
    float sum = 0.0f;
    for (int k = 0; k < K; k++) {
        sum += A[k] * aljabr_metal_bf16_to_f32(weight[k]);
    }
    return sum;
}
...
BOOL aljabr_metal_validate_mps_matvec_half_output(const float* C,
                                                  ...
                                                  int K,
                                                  int N) {
    if (aljabr_metal_env_truthy("ALJABR_METAL_MPS_MATVEC_SKIP_VALIDATE")) {
        return YES;
    }
    int samples = aljabr_metal_env_int_or_default("ALJABR_METAL_MPS_MATVEC_VALIDATE_SAMPLES", 8);
    if (samples <= 0) {
        samples = 1;
    }
    if (samples > N) {
    ...
    float abs_tol = 0.08f;
    float rel_tol = 0.03f;
    const char* abs_env = getenv("ALJABR_METAL_MPS_MATVEC_VALIDATE_ABS_TOL");
    const char* rel_env = getenv("ALJABR_METAL_MPS_MATVEC_VALIDATE_REL_TOL");
    ...
    for (int i = 0; i < samples; i++) {
        ...
        if (diff > limit) {
            if (aljabr_metal_env_truthy("ALJABR_METAL_MPS_MATVEC_DEBUG")) {
                fprintf(stderr,
                        "[aljabr-metal] MPS matvec validation failed row=%d K=%d N=%d actual=%g expected=%g diff=%g limit=%g\n",
...
BOOL aljabr_metal_validate_mps_matvec_bf16_output(const float* C,
    ...
    for (int i = 0; i < samples; i++) {
        ...
        if (diff > limit) {
            if (aljabr_metal_env_truthy("ALJABR_METAL_MPS_MATVEC_DEBUG")) {
                fprintf(stderr,
                        "[aljabr-metal] MPS BF16 matvec validation mismatch row=%d actual=%f expected=%f diff=%f limit=%f K=%d N=%d\n",
