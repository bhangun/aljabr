/**
 * aljabr_metal_gated_ffn.m — Metal gated FFN public kernels.
 */

#import "aljabr_metal_gated_ffn.h"
...
static int aljabr_metal_gated_ffn_half_impl(void* C,
                           const void* A,
                           const void* gateW,
                           const void* upW,
                           const void* downW,
...
int aljabr_metal_swiglu_ffn_half(void* C,
                           const void* A,
                           const void* gateW,
                           const void* upW,
                           const void* downW,
                           int M, int input_dim, int intermediate_dim, int output_dim,
                           int is_bf16) {
    return aljabr_metal_gated_ffn_half_impl(C, A, gateW, upW, downW,
            M, input_dim, intermediate_dim, output_dim, is_bf16,
            aljabr_metal_pipelines()->silu_ffn);
}

int aljabr_metal_geglu_ffn_half(void* C,
                           const void* A,
                           const void* gateW,
                           const void* upW,
                           const void* downW,
    ...
    return aljabr_metal_gated_ffn_half_impl(C, A, gateW, upW, downW,
            M, input_dim, intermediate_dim, output_dim, is_bf16,
            aljabr_metal_pipelines()->gelu_ffn);
}

static int aljabr_metal_gated_ffn_matvec_half_impl(void* C,
                           const void* A,
                           const void* gateW,
                           const void* upW,
                           const void* downW,
    ...
    BOOL useBf16FusedGateX4 = is_bf16
            && aljabr_metal_env_truthy("ALJABR_METAL_ENABLE_BF16_FUSED_GATED_PAIR_X4")
            && !aljabr_metal_env_truthy("ALJABR_METAL_DISABLE_BF16_FUSED_GATED_PAIR_X4")
    ...
    if (is_bf16 && useFusedGateProjection && pipelines->matvec_bf16_gated_pair_simd != nil
            && !useBf16FusedGateX4
            && !aljabr_metal_env_truthy("ALJABR_METAL_DISABLE_BF16_FUSED_GATED_PAIR_SIMD")) {
    ...
    @autoreleasepool {
        ...
        id<MTLComputePipelineState> pairPipeline =
                pairThreads == ALJABR_MATVEC_THREADS_128 ? pairPipeline128 : pairPipeline256;
        ...
        id<MTLComputePipelineState> downPipeline =
                downThreads == ALJABR_MATVEC_THREADS_128 ? downPipeline128 : downPipeline256;
...
static int aljabr_metal_gated_ffn_matvec_rows_bf16_impl(void* C,
                           const void* A,
                           const void* gateW,
                           const void* upW,
                           const void* downW,
    ...
    if (pairPipeline.maxTotalThreadsPerThreadgroup < ALJABR_MATVEC_THREADS_256
            || downPipeline.maxTotalThreadsPerThreadgroup < ALJABR_MATVEC_THREADS_256) {
        return -3;
    }
    ...
    @autoreleasepool {
        ...
        uint32_t outputN = (uint32_t)output_dim;
        uint32_t activationKind = (uint32_t)activation_kind;
        NSUInteger threads = ALJABR_MATVEC_THREADS_256;
...
int aljabr_metal_bf16_ffn_matvec_rows_variant(void) {
    if (!g_initialized) return -1;
    AljabrMetalPipelines* pipelines = aljabr_metal_pipelines();
...
int aljabr_metal_swiglu_ffn_matvec_half(void* C,
                           const void* A,
                           const void* gateW,
                           const void* upW,
                           const void* downW,
                           int input_dim, int intermediate_dim, int output_dim) {
    return aljabr_metal_gated_ffn_matvec_half_impl(C, A, gateW, upW, downW,
            input_dim, intermediate_dim, output_dim, 0, 1,
            aljabr_metal_pipelines()->silu_ffn);
}

int aljabr_metal_geglu_ffn_matvec_half(void* C,
                           const void* A,
                           const void* gateW,
                           const void* upW,
                           const void* downW,
...
int aljabr_metal_swiglu_ffn_matvec_bf16(void* C,
                           const void* A,
                           const void* gateW,
                           const void* upW,
                           const void* downW,
...
int aljabr_metal_geglu_ffn_matvec_bf16(void* C,
                           const void* A,
                           const void* gateW,
                           const void* upW,
                           const void* downW,
...
int aljabr_metal_swiglu_ffn_matvec_rows_bf16(void* C,
                           const void* A,
                           const void* gateW,
                           const void* upW,
                           const void* downW,
                           int M, int input_dim, int intermediate_dim, int output_dim) {
    return aljabr_metal_gated_ffn_matvec_rows_bf16_impl(C, A, gateW, upW, downW,
            M, input_dim, intermediate_dim, output_dim, 1);
}

int aljabr_metal_geglu_ffn_matvec_rows_bf16(void* C,
                           const void* A,
                           const void* gateW,
                           const void* upW,
                           const void* downW,
