/**
 * aljabr_metal_runtime.m — exported runtime/device helpers for the Metal dylib.
 */

#import "aljabr_metal_runtime.h"
...
int aljabr_metal_init(void) {
    ...
    aljabr_metal_mps_matmul_configure_from_env();
    aljabr_metal_mps_cache_init(aljabr_metal_env_truthy("ALJABR_METAL_DISABLE_MPS_CACHE"));
    aljabr_metal_mps_matvec_policy_init();
    BOOL enable_elementwise_kernels = aljabr_metal_env_truthy("ALJABR_METAL_ENABLE_ELEMENTWISE_KERNELS");
    BOOL explicit_disable_elementwise = aljabr_metal_env_truthy("ALJABR_METAL_DISABLE_ELEMENTWISE_KERNELS");
    aljabr_metal_elementwise_set_enabled(enable_elementwise_kernels && !explicit_disable_elementwise);
    aljabr_metal_compile_runtime_pipelines(aljabr_metal_pipelines());
    g_initialized = YES;
    return 0;
}
...
int aljabr_metal_set_mps_matvec_enabled(int enabled) {
    return aljabr_metal_mps_matvec_set_enabled(enabled);
}

int aljabr_metal_set_mps_matvec_autotune_enabled(int enabled) {
    return aljabr_metal_mps_matvec_set_autotune_enabled(enabled);
}

int aljabr_metal_set_mps_matvec_max_inner(int max_inner) {
    return aljabr_metal_mps_matvec_set_max_inner(max_inner);
}

int aljabr_metal_set_mps_matvec_max_output(int max_output) {
    return aljabr_metal_mps_matvec_set_max_output(max_output);
}

int aljabr_metal_set_mps_matvec_autotune_max_output(int max_output) {
    return aljabr_metal_mps_matvec_set_autotune_max_output(max_output);
}
...
static inline BOOL aljabr_metal_argmax_rejected(int id,
                                                int reject0,
                                                int reject1,
                                                int reject2,
                                                int reject3,
...
static int aljabr_metal_argmax_scalar_f32(const float* values,
                                          int n,
                                          int reject0,
                                          int reject1,
                                          int reject2,
    ...
    for (int i = 0; i < n; i++) {
        if (aljabr_metal_argmax_rejected(i, reject0, reject1, reject2, reject3, reject4, reject5, reject6, reject7)) {
            continue;
        }
...
static int aljabr_metal_argmax_collect_rejections(int n,
                                                  int out[8],
                                                  int reject0,
                                                  int reject1,
                                                  int reject2,
...
static int aljabr_metal_argmax_vdsp_range_f32(const float* values,
                                              int start,
                                              int end,
                                              float* best_value,
                                              int* best_index) {
...
static int aljabr_metal_argmax_vdsp_f32(const float* values,
                                        int n,
                                        int reject0,
                                        int reject1,
                                        int reject2,
                                        ...
                                        int reject7) {
    int rejections[8] = { 0 };
    int rejection_count = aljabr_metal_argmax_collect_rejections(n, rejections,
            reject0, reject1, reject2, reject3, reject4, reject5, reject6, reject7);
    float best_value = -INFINITY;
    int best_index = -1;
    int start = 0;
    for (int i = 0; i < rejection_count; i++) {
        if (aljabr_metal_argmax_vdsp_range_f32(values, start, rejections[i], &best_value, &best_index) < 0) {
            return INT_MIN;
        }
        start = rejections[i] + 1;
    }
    if (aljabr_metal_argmax_vdsp_range_f32(values, start, n, &best_value, &best_index) < 0) {
        return INT_MIN;
    }
    return best_index;
}
...
int aljabr_metal_argmax_f32(const void* logits,
    ...
    int vdsp_best = aljabr_metal_argmax_vdsp_f32(values, n, reject0, reject1, reject2, reject3,
            reject4, reject5, reject6, reject7);
    if (vdsp_best != INT_MIN) {
        return vdsp_best;
    }
    return aljabr_metal_argmax_scalar_f32(values, n, reject0, reject1, reject2, reject3,
            reject4, reject5, reject6, reject7);
}
