/**
 * aljabr_metal_buffers.m — shared Metal buffer helpers.
 */
...
id<MTLBuffer> aljabr_metal_wrap_weight_ptr(const void* ptr, size_t bytes) {
    ...
    if (!aljabr_metal_env_truthy("ALJABR_METAL_ENABLE_WEIGHT_BUFFER_CACHE")
            || aljabr_metal_env_truthy("ALJABR_METAL_DISABLE_WEIGHT_BUFFER_CACHE")) {
        return aljabr_metal_wrap_ptr((void*)ptr, bytes);
    }
    ...
    id<MTLBuffer> wrapped = aljabr_metal_wrap_ptr((void*)ptr, bytes);
    if (wrapped == nil) {
        return nil;
    }
    ...
    @synchronized([NSMutableDictionary class]) {
        ...
        int max_entries = aljabr_metal_env_int_or_default("ALJABR_METAL_WEIGHT_BUFFER_CACHE_MAX_ENTRIES", 4096);
