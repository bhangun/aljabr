package tech.kayys.aljabr.rocm.runner;
...
import tech.kayys.aljabr.plugin.runner.RunnerInitializationException;
import tech.kayys.aljabr.extension.AbstractAljabrRunner;
import tech.kayys.aljabr.rocm.binding.RocmHipBinding;
import tech.kayys.aljabr.rocm.binding.RocmHipCpuFallback;
 ...
 * {@link tech.kayys.aljabr.extension.metal.runner.MetalRunner} but uses HIP
 * instead of Metal:
 * <ol>
 *   <li>RMS Norm — {@code aljabr_rocm_rmsnorm}</li>
 *   <li>QKV projection — {@code aljabr_rocm_gemm}</li>
 *   <li>Paged attention — {@code aljabr_rocm_paged_attention} (hipified FA)</li>
 *   <li>SiLU FFN — {@code aljabr_rocm_silu_ffn}</li>
 * </ol>
 *
 ...
 * <h3>Config</h3>
 * <pre>
 *   aljabr.runners.rocm.enabled=false
 *   aljabr.runners.rocm.library-path=/opt/rocm/lib/libamdhip64.so
 *   aljabr.runners.rocm.kernel-path=/opt/aljabr/lib/aljabr_rocm_gfx942.hsaco
 *   aljabr.runners.rocm.device-id=0
 *   aljabr.runners.rocm.num-layers=32
 *   aljabr.runners.rocm.num-heads=32
 *   aljabr.runners.rocm.num-heads-kv=8
 *   aljabr.runners.rocm.head-dim=128
 *   aljabr.runners.rocm.model-dim=4096
 *   aljabr.runners.rocm.ffn-dim=14336
 *   aljabr.runners.rocm.vocab-size=32000
 * </pre>
 *
 * <h3>Build HIP kernels</h3>
 * <pre>
 *   make -C src/main/cpp/rocm AMDGPU_TARGET=gfx942
 *   # Output: target/native/linux-x86_64/aljabr_rocm_gfx942.hsaco
 * </pre>
 */
@ApplicationScoped
public class RocmRunner extends AbstractAljabrRunner {
    ...
    @ConfigProperty(name = "aljabr.runners.rocm.enabled",      defaultValue = "false")
    boolean enabled;

    @ConfigProperty(name = "aljabr.runners.rocm.library-path",
                    defaultValue = "/opt/rocm/lib/libamdhip64.so")
    String libraryPath;

    @ConfigProperty(name = "aljabr.runners.rocm.kernel-path",
                    defaultValue = "/opt/aljabr/lib/aljabr_rocm_gfx942.hsaco")
    String kernelPath;

    @ConfigProperty(name = "aljabr.runners.rocm.device-id",    defaultValue = "0")
    int deviceId;

    @ConfigProperty(name = "aljabr.runners.rocm.num-layers",   defaultValue = "32")
    int numLayers;

    @ConfigProperty(name = "aljabr.runners.rocm.num-heads",    defaultValue = "32")
    int numHeads;

    @ConfigProperty(name = "aljabr.runners.rocm.num-heads-kv", defaultValue = "8")
    int numHeadsKv;

    @ConfigProperty(name = "aljabr.runners.rocm.head-dim",     defaultValue = "128")
    int headDim;

    @ConfigProperty(name = "aljabr.runners.rocm.model-dim",    defaultValue = "4096")
    int modelDim;

    @ConfigProperty(name = "aljabr.runners.rocm.ffn-dim",      defaultValue = "14336")
    int ffnDim;

    @ConfigProperty(name = "aljabr.runners.rocm.vocab-size",   defaultValue = "32000")
    int vocabSize;
    ...
    public void initialize(ModelManifest modelManifest, RunnerConfiguration config)
        ...
        if (!enabled) throw new RunnerInitializationException(
                ...
                "ROCm runner disabled (aljabr.runners.rocm.enabled=false)");
        ...
        if (!isNull(hipModule)) {
            fnRmsNorm        = hip.moduleGetFunction(hipModule, "aljabr_rocm_rmsnorm");
            fnGemm           = hip.moduleGetFunction(hipModule, "aljabr_rocm_gemm");
            fnPagedAttention = hip.moduleGetFunction(hipModule, "aljabr_rocm_paged_attention");
            fnSiluFfn        = hip.moduleGetFunction(hipModule, "aljabr_rocm_silu_ffn");
