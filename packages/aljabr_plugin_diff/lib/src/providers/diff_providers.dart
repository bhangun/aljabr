...
class FileDiffsNotifier extends StateNotifier<List<FileDiff>> {
  ...
  void setAllHunksInFile(String path, HunkStatus status) {
    state = [
      for (final file in state)
        if (file.path == path)
          file.copyWith(
              hunks: [for (final h in file.hunks) h.copyWith(status: status)])
        else
          file,
    ];
  }
  ...
  void revertAll() {
    state = [
      for (final file in state)
        file.copyWith(hunks: [
          for (final h in file.hunks) h.copyWith(status: HunkStatus.rejected)
        ]),
    ];
  }
}
        ...
        DiffHunk(
          ...
          lines: [
            ...
            DiffLine(
                ...
                content: '    ports: ["5432:5432"]',
                oldLineNo: 11),
            DiffLine(
                type: DiffLineType.addition,
                content: '    ports: ["5433:5432"]',
                newLineNo: 11),
            DiffLine(
              type: DiffLineType.context,
              content: '    environment:',
      ...
      hunks: [
        DiffHunk(
          ...
          lines: [
            ...
            DiffLine(
                type: DiffLineType.addition,
                content: '  ON sessions (project_id);',
                newLineNo: 2),
          ],
        ),
      ],
...
List<FileDiff> _diffSeedFor(String sessionId) =>
    _seedDiffs[sessionId] ?? const [];
...
final diffSummaryProvider =
    Provider.family<(int additions, int deletions), String>(
  (ref, sessionId) {
